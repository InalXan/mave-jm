                ...
                <form @submit.prevent="handleSubmit" class="w-full bg-white shadow-md p-6">
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full md:w-full px-3 mb-6">
                            ...
                            <input v-model="title" class="appearance-none block w-full bg-white text-gray-900 font-medium border border-gray-400 rounded-lg py-3 px-3 leading-tight focus:outline-none focus:border-[#98c01d]" type="text" name="name" placeholder="Category Name" required />
                        </div>
                        <div class="w-full px-3 mb-6">
                            <textarea v-model="description" textarea rows="4" class="appearance-none block w-full bg-white text-gray-900 font-medium border border-gray-400 rounded-lg py-3 px-3 leading-tight focus:outline-none focus:border-[#98c01d]" type="text" name="description" required > </textarea>
                        </div>

                        <div class="w-full md:w-full px-3 mb-6">
                            <button class="appearance-none block w-full bg-primary text-gray-100 font-bold border border-gray-200 rounded-lg py-3 px-3 leading-tight hover:bg-primary/50 focus:outline-none focus:bg-white focus:border-gray-500"
                            >{{ selectedContent ? 'Güncelle' : 'Ekle' }}</button>
                            <button v-if="selectedContent" @click="selectedContent = null; title = ''; description = ''" type="button" class="cancel">İptal</button>
                        </div>

                        <div class="w-full px-3 mb-8">
                            <label class="mx-auto cursor-pointer flex w-full max-w-lg flex-col items-center justify-center rounded-xl border-2 border-dashed border-primary bg-second p-6 text-center" htmlFor="dropzone-file">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                        ...
                        <thead class="text-sm font-semibold uppercase text-gray-800 bg-gray-50 mx-auto">
                            ...
                            <tr v-for="content in contents" :key="content._id">
                                ...
                                <td class="p-2">
                                    ...
                                    <button @click="handleEdit(content)" class="rounded-md hover:bg-primary/60 text-primary cursor-pointer p-2 flex justify-between items-center">
                                        <span><FaEdit class="w-4 h-4 mr-1"/>
                                        </span> Edit
                                      </button>
                                    <button class="rounded-md hover:bg-[crimson] text-[crimson] hover:text-[crimson] hover:text-second cursor-pointer p-2 flex justify-between items-center" @click="handleDelete(content._id)">
...
const handleSubmit = async () => {
    if (selectedContent.value) {
        await updateContent(selectedContent.value._id, { title: title.value, description: description.value });
    } else {
